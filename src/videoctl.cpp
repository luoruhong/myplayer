...
//            qDebug() << "1 audio_buf_size: " << audio_size;
            ...
            if(!pVideoCtl->is_normal_playback_rate() && is->audio_buf)
            {
                ...
                // 计算处理后的点数
                int out_ret = sonicWriteFloatToStream(pVideoCtl->audio_speed_convert,
                                                      (float *)is->audio_buf,
                                                      actual_out_samples);
                int numSamples =  sonicSamplesAvailable(pVideoCtl->audio_speed_convert);
                int out_size = (numSamples) * 4 *2;      // 2通道 2字节一个采样点
                av_fast_malloc(&is->audio_buf1, &is->audio_buf1_size, out_size);
                if(out_ret)
                {
                    // 从流中读取处理好的数据
                    int sonic_samples = sonicReadFloatFromStream(pVideoCtl->audio_speed_convert,
                                                                 (float *)is->audio_buf1,
                                                                 numSamples);
                    is->audio_buf = is->audio_buf1;
//                    qDebug() << "mdy numSamples: " << numSamples;
//                    qDebug() << "orig audio_buf_size: " << is->audio_buf_size;
                    is->audio_buf_size = sonic_samples * is->audio_tgt.channels *4;
